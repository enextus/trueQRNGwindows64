...
public class App {
    ...
    public static final int INT_AMOUNT = 41;
...
/*
    private static void getAndPrintIntegerArray(QuantumRandomNumberGenerator lib) {
        ...
        if (getArrayResult != 0) {
            ...
            // Define the new range for values
            int newMin = -100;
            int newMax = 100;

            // Find the min and max values in the original array
            int min = Arrays.stream(intArray).min().orElse(Integer.MIN_VALUE);
            int max = Arrays.stream(intArray).max().orElse(Integer.MAX_VALUE);

            // Apply the range conversion formula to each value in the array
            int[] convertedArray = Arrays.stream(intArray)
                    .map(value -> (value - min) * (newMax - newMin) / (max - min) + newMin)
                    .toArray();
